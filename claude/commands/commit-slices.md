...
2. **Get the authoritative diff.** The script lives at
   `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/slice-commits/scripts/hunk_slice.py`.
   Run:
   ```
   python3 <script> show
   ```
   **On macOS only**, wrap every invocation in `nix-shell -p python3 --run "..."`:
   bare `python3` there resolves to a broken macOS stub (`/usr/bin/python3`) that
   errors with `tool 'python3' not found`, so a plain-`python3` check passes but
   execution fails. On Linux and in cloud sessions, plain `python3` is correct.

   Read the whole `F/H/L` inventory — IDs are positional to *this* invocation, so
   build the plan against this exact output.
...
- If nothing consumes a category (e.g. it only ever tracked files now deleted or
  ...
  prune it.
- In a cloud session the whole run usually lands as one branch and one PR, so
  slicing buys less there than it does locally.
